...
public class PinnedSectionListView extends ListView {
    ...
    private GradientDrawable mShadowDrawable;// numLine 410 去阴影
        ...
        public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
            ...
            if (isFirstVisibleItemSection) {
                ...
                if (sectionView.getTop() == getPaddingTop()) { // view sticks to the top, no need for pinned shadow
                    destroyPinnedShadow();
                	if(getChildAt(0) != null) getChildAt(0).setVisibility(View.VISIBLE);
            	...
            	if(convertView != null) convertView.setVisibility(View.VISIBLE);
                int sectionPosition = findCurrentSectionPosition(firstVisibleItem);
                if (sectionPosition > -1) { // we have section position
                    ensureShadowForPosition(sectionPosition, firstVisibleItem, visibleItemCount);
                    if (getChildAt(1) != null) getChildAt(1).setVisibility(View.VISIBLE);
	...
	void createPinnedShadow(int position) {
		...
		ViewGroup.LayoutParams layoutParamsTemp = (LayoutParams) pinnedView.getLayoutParams();
		if (layoutParamsTemp == null) {
			layoutParamsTemp = generateDefaultLayoutParams();
		}
		AbsListView.LayoutParams layoutParams = null;
	    if(layoutParamsTemp instanceof AbsListView.LayoutParams){
	    	layoutParams = (LayoutParams) layoutParamsTemp;
	    }else{
	    	layoutParams = (LayoutParams) generateLayoutParams(layoutParamsTemp);
	    }
